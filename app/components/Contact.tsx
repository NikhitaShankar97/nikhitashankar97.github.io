        ...
        <div className="grid grid-cols-[1fr_1fr] gap-16 items-start max-lg:grid-cols-1 max-lg:gap-10">
          ...
          <div>
            {submitted ? (
              ...
              <motion.form action={portfolioData.formspreeEndpoint} method="POST" className="space-y-3" onSubmit={(e) => { if (!form.name || !form.email || !form.message) { e.preventDefault(); return }; setSubmitted(true) }} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
                <div className="grid grid-cols-2 gap-3 max-md:grid-cols-1">
                  <input type="text" name="name" placeholder="Your name" required value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full px-4 py-3 bg-white/[0.02] border border-white/[0.05] rounded-xl text-sm text-white outline-none transition-all focus:border-accent/40 focus:bg-white/[0.03] placeholder:text-zinc-600" />
                  <input type="email" name="_replyto" placeholder="Your email" required value={form.email} onChange={e => setForm({...form, email: e.target.value})} className="w-full px-4 py-3 bg-white/[0.02] border border-white/[0.05] rounded-xl text-sm text-white outline-none transition-all focus:border-accent/40 focus:bg-white/[0.03] placeholder:text-zinc-600" />
                </div>
                <textarea name="message" placeholder="What would you like to discuss?" rows={4} required value={form.message} onChange={e => setForm({...form, message: e.target.value})} className="w-full px-4 py-3 bg-white/[0.02] border border-white/[0.05] rounded-xl text-sm text-white outline-none resize-y transition-all focus:border-accent/40 focus:bg-white/[0.03] placeholder:text-zinc-600" />
